...
import { PinterestConfig, AdAccount, PinterestCampaign, TargetLocation, AgeGroup, OptimizationGoal, ConversionEvent, PinterestSyncLink, MetaSyncLink, GoogleSyncLink } from '../../types';
import { Share2, Plus, Search, Layers, ArrowRight, Save, Activity, Trash2, LayoutTemplate, ChevronDown, Filter, AlertTriangle, Info, Target, Facebook } from 'lucide-react';

interface PinterestSyncProps {
  config: PinterestConfig;
  onChange: (newConfig: PinterestConfig) => void;
...
const MOCK_AD_ACCOUNTS: AdAccount[] = [
  { id: 'act_123', name: 'ReBoss Official Ads', currency: 'EUR' },
  { id: 'act_456', name: 'ReBoss US Backup', currency: 'USD' },
];

const MOCK_CAMPAIGNS: PinterestCampaign[] = [
  { id: 'cmp_1', name: 'Summer Sale 2024 - ROAS', status: 'ACTIVE' },
  { id: 'cmp_2', name: 'Retargeting - All Users', status: 'ACTIVE' },
  { id: 'cmp_3', name: 'Winter Clearance', status: 'PAUSED' },
];
...
export const PinterestSync: React.FC<PinterestSyncProps> = ({ 
  config, 
  onChange,
  metaLinks = [],
  googleLinks = []
  ...
  const [formState, setFormState] = useState({
    ...
    selectedBatch: 1,
    isCampaignNameDirty: false, // Track if user manually edited the name
  });
  ...
  const handleCreateLink = () => {
    ...
    const newLink: PinterestSyncLink = {
  ...
  const canSave = formState.selectedCollectionId && (
    ...
    return (
      <div className="flex flex-col items-center justify-center h-[600px] animate-in fade-in zoom-in-95 duration-500">
        ...
        <p className="text-zinc-400 text-center max-w-md mb-8">
          Connect your Ad Account to automatically sync high-performing products into targeted campaigns.
        </p>
        <button 
          onClick={() => onChange({...config, isConnected: true, selectedAdAccountId: MOCK_AD_ACCOUNTS[0].id, activeLinks: []})}
          className="bg-[#E60023] hover:bg-[#ad081b] text-white px-8 py-3 rounded-full font-medium transition-all shadow-lg hover:shadow-[#E60023]/25 flex items-center gap-2"
        >
           ...
           <div className="relative group">
              ...
              <div className="absolute right-0 top-full mt-2 w-64 p-4 bg-zinc-900 border border-zinc-800 rounded-xl shadow-2xl opacity-0 group-hover:opacity-100 transition-opacity z-50 pointer-events-none transform translate-y-2 group-hover:translate-y-0 duration-200">
                 ...
                 <div className="bg-zinc-950/50 rounded border border-zinc-800 p-2 text-[10px] text-zinc-500">
                    <span className="text-zinc-300 font-medium">Tip:</span> Smaller batches (e.g. 50) ensure better stability but take longer.
                 </div>
              </div>
           </div>
                       ...
                       filteredLinks.map(link => (
                          <tr key={link.id} className="group hover:bg-zinc-900/50 transition-colors">
                             ...
                             <td className="px-2 py-3 text-center align-top">
                                <span className="inline-flex items-center justify-center px-1.5 py-0.5 rounded text-[10px] font-mono bg-zinc-800/50 text-zinc-400 border border-zinc-800">
                                   #{link.batchNumber}
                                </span>
                             </td>
                    ...
                    <div className="flex items-center justify-between">
                       ...
                       <div className="flex bg-zinc-950 p-1 rounded-lg border border-zinc-800">
                          ...
                          <button 
                             ...
                             className={`px-3 py-1 rounded text-xs font-medium transition-all ${formState.campaignMode === 'NEW' ? 'bg-red-500 text-white shadow-sm' : 'text-zinc-500 hover:text-zinc-300'}`}
                          >
                             Create New
                          </button>
                       </div>
                       ...
                       <div className="bg-zinc-950 border border-zinc-800 rounded-xl p-5 space-y-6 animate-in slide-in-from-top-2">
                          ...
                          <div>
                             ...
                             <input 
                                ...
                                className="w-full bg-zinc-900 border border-zinc-800 rounded-lg py-2 px-3 text-sm text-white focus:outline-none focus:border-red-500/50"
                             />
                          </div>
                             ...
                             <div className="space-y-4">
                                ...
                                <div className="space-y-2">
                                   ...
                                   <input 
                                      ...
                                      className="w-full h-1.5 bg-zinc-800 rounded-lg appearance-none cursor-pointer [&::-webkit-slider-thumb]:appearance-none [&::-webkit-slider-thumb]:w-3.5 [&::-webkit-slider-thumb]:h-3.5 [&::-webkit-slider-thumb]:bg-red-500 [&::-webkit-slider-thumb]:rounded-full"
                                   />
                                </div>
                                ...
                                <div className="flex flex-wrap gap-1.5">
                                   {LOCATIONS.map(loc => (
                                      <button
                                         ...
                                         className={`px-2 py-1 rounded text-[10px] font-bold border ${config.defaultCampaignSettings.targetLocations.includes(loc) ? 'bg-red-500/10 border-red-500/30 text-red-400' : 'bg-zinc-900 border-zinc-800 text-zinc-500'}`}
                                      >
                                         {loc}
                                      </button>
                                   ))}
