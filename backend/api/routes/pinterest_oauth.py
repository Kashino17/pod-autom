...
def sync_campaigns():
    """Sync Pinterest Campaigns to database for a shop"""
    try:
        data = request.json
        shop_id = data.get('shop_id')
        ...
        # Get selected ad account for this shop
        ad_account_result = supabase.table('pinterest_ad_accounts').select('pinterest_account_id').eq('shop_id', shop_id).eq('is_selected', True).execute()

        if not ad_account_result.data:
            return jsonify({'error': 'No ad account selected. Please select an ad account first.'}), 400

        pinterest_account_id = ad_account_result.data[0]['pinterest_account_id']
        ...
        # Sync campaigns to Supabase - only ACTIVE ones
        synced_count = 0
        for campaign in all_campaigns:
            ...
            if status == 'ACTIVE':
                supabase.table('pinterest_campaigns').upsert({
                    ...
                    'status': status,
                    'daily_budget': campaign.get('daily_spend_cap') / 1000000 if campaign.get('daily_spend_cap') else None,
                    'synced_at': datetime.now(timezone.utc).isoformat()
