...
## Ziel
Erstellen aller POD AutoM Datenbank-Tabellen in Supabase mit vollständigem Schema, Row Level Security, Trigger-Funktionen und Sample Data.

## Technische Anforderungen
- Supabase PostgreSQL 15+
- Bestehende Supabase-Instanz (gleiche wie ReBoss)
- Alle Tabellen mit Prefix `pod_autom_`
...
│       │                                                                  │
│       ├──── pod_autom_subscriptions (1:1)                               │
│       │                                                                  │
│       └──── pod_autom_shops (1:N)                                       │
│                   │                                                      │
│                   └──── pod_autom_settings (1:1)                        │
│                              │                                           │
│                              ├──── pod_autom_niches (1:N)               │
│                              │                                           │
│                              └──── pod_autom_prompts (1:N)              │
│                                                                          │
│  pod_autom_products (Shop-bezogen, für Tracking)                        │
│                                                                          │
│  pod_autom_ad_platforms (User-bezogen, OAuth Credentials)               │
│                                                                          │
│  pod_autom_activity_log (Audit Trail)                                   │
│                                                                          │
│  pod_autom_catalog (Global, keine User-Zuordnung)                       │
│                                                                          │
└─────────────────────────────────────────────────────────────────────────┘
```
...
### Vollständige Migration (alle Tabellen)
...
-- Beginne Transaktion für atomare Migration
BEGIN;
...
CREATE TABLE IF NOT EXISTS pod_autom_shops (
  ...
  -- Shopify OAuth
  access_token TEXT, -- Verschlüsselt speichern!
...
CREATE TABLE IF NOT EXISTS pod_autom_settings (
  ...
  compare_at_price_multiplier DECIMAL(4,2) DEFAULT 1.30, -- Für Streichpreise
...
CREATE TABLE IF NOT EXISTS pod_autom_niches (
  ...
  priority INTEGER DEFAULT 0, -- Höhere Priorität = mehr Produkte
...
CREATE TABLE IF NOT EXISTS pod_autom_prompts (
  ...
  prompt_name VARCHAR(255), -- Optionaler Name für den Prompt
...
CREATE TABLE IF NOT EXISTS pod_autom_activity_log (
  ...
  -- Context
  details JSONB DEFAULT '{}', -- Zusätzliche Details
  ip_address INET,
  user_agent TEXT,
...
-- Partitionierung für Activity Log (optional, für große Datenmengen)
-- CREATE TABLE pod_autom_activity_log_y2026m01 PARTITION OF pod_autom_activity_log
...
CREATE TABLE IF NOT EXISTS pod_autom_catalog (
  ...
  -- Specifications
  materials TEXT, -- z.B. "100% Baumwolle, 180g/m²"
...
-- Service Role kann alles (für Stripe Webhooks)
...
(
  ...
  'Hochwertiges Premium T-Shirt aus 100% Bio-Baumwolle. Perfekt für hochauflösende Drucke.',
  ARRAY['XS', 'S', 'M', 'L', 'XL', 'XXL', '3XL'],
  '[{"name": "Schwarz", "hex": "#000000", "available": true},
    {"name": "Weiß", "hex": "#FFFFFF", "available": true},
  ...
  '100% Bio-Baumwolle, 180g/m², Rundhals',
...
(
  ...
  'Kuscheliger Hoodie mit Kängurutasche. Ideal für kältere Tage.',
  ARRAY['S', 'M', 'L', 'XL', 'XXL'],
  '[{"name": "Schwarz", "hex": "#000000", "available": true},
    {"name": "Navy", "hex": "#1e3a5f", "available": true},
    {"name": "Grau Meliert", "hex": "#6b7280", "available": true},
    {"name": "Forest Green", "hex": "#228B22", "available": true}]'::jsonb,
  '80% Baumwolle, 20% Polyester, 300g/m²',
...
(
  ...
  '80% Baumwolle, 20% Polyester, 280g/m²',
...
(
  ...
  'Nachhaltige Stofftasche aus Bio-Baumwolle. Perfekt für den Alltag.',
  ARRAY['One Size'],
  '[{"name": "Natur", "hex": "#f5f5dc", "available": true},
    {"name": "Schwarz", "hex": "#000000", "available": true}]'::jsonb,
  '100% Bio-Baumwolle, 140g/m²',
...
(
  ...
  'Hochqualitatives Poster auf 200g/m² Fotopapier. Brillante Farben.',
  ARRAY['A3 (29.7 x 42 cm)'],
  '[]'::jsonb,
  '200g/m² Premium Fotopapier, matt',
...
(
  ...
  'Großformatiges Poster auf Premium Fotopapier.',
...
(
  ...
  'Klassische Keramiktasse für den morgendlichen Kaffee.',
  ARRAY['325ml'],
  '[{"name": "Weiß", "hex": "#FFFFFF", "available": true}]'::jsonb,
  'Keramik, spülmaschinenfest',
...
2. Wähle das Projekt aus
3. Navigiere zu "SQL Editor"

### 2. Migration ausführen
1. Erstelle eine neue Query
...
WHERE table_schema = 'public' AND table_name LIKE 'pod_autom_%'
ORDER BY table_name;
...
WHERE schemaname = 'public' AND tablename LIKE 'pod_autom_%';

-- Catalog Daten prüfen
SELECT product_code, display_name, base_price FROM pod_autom_catalog
ORDER BY sort_order;

-- Triggers prüfen
SELECT trigger_name, event_manipulation, event_object_table
FROM information_schema.triggers
WHERE trigger_name LIKE '%pod_autom%';
```
...
Dies aktualisiert `src/lib/database.types.ts` automatisch mit allen neuen Tabellen.
...
### Schema Tests
- [ ] Alle 9 Tabellen wurden erstellt
- [ ] Alle Indexes wurden erstellt
- [ ] RLS ist für alle Tabellen aktiviert
- [ ] Alle RLS Policies sind korrekt konfiguriert
- [ ] Alle Trigger wurden erstellt

### Funktionale Tests
- [ ] `updated_at` Trigger funktioniert (Update einer Zeile → updated_at ändert sich)
- [ ] Auto-Create Settings Trigger funktioniert (Shop erstellen → Settings werden automatisch erstellt)
- [ ] Niche Slug Generation funktioniert
- [ ] Service Role kann alle Tabellen ohne Einschränkung lesen/schreiben

### Sample Data Tests
- [ ] Catalog enthält 8 Produkte
- [ ] Alle Produkte haben korrekte Daten

### RLS Tests (als normaler User)
- [ ] User kann nur eigene Subscriptions sehen
- [ ] User kann nur eigene Shops sehen
- [ ] User kann nur eigene Settings/Niches/Prompts sehen
- [ ] User kann Catalog sehen (öffentlich)
- [ ] User kann KEINE fremden Daten sehen

---

## Abhängigkeiten
- Supabase Projekt muss existieren
- Supabase Service Key für Backend-Jobs

## Geschätzte Dauer
- Erfahrener Entwickler: 15-20 Minuten
- Anfänger: 45-60 Minuten
...
```sql
-- ACHTUNG: Löscht alle POD AutoM Tabellen und Daten!
DROP TABLE IF EXISTS pod_autom_activity_log CASCADE;
DROP TABLE IF EXISTS pod_autom_products CASCADE;
DROP TABLE IF EXISTS pod_autom_prompts CASCADE;
DROP TABLE IF EXISTS pod_autom_niches CASCADE;
DROP TABLE IF EXISTS pod_autom_settings CASCADE;
DROP TABLE IF EXISTS pod_autom_ad_platforms CASCADE;
DROP TABLE IF EXISTS pod_autom_shops CASCADE;
DROP TABLE IF EXISTS pod_autom_subscriptions CASCADE;
DROP TABLE IF EXISTS pod_autom_catalog CASCADE;

-- Trigger Functions
DROP FUNCTION IF EXISTS update_updated_at_column CASCADE;
DROP FUNCTION IF EXISTS create_default_settings CASCADE;
DROP FUNCTION IF EXISTS generate_niche_slug CASCADE;
DROP FUNCTION IF EXISTS reset_daily_creation_count CASCADE;
```
